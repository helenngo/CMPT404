...
Note that there is an additional input variable for the pla function, $seedling$, that changes as the trials change. This variable is passed into the pla function and then choose\_miscl\_point function so that the seed for random will always be different. A histogram of the iterations needed for a valid final hypothesis for the data set, such that each misclassified point is picked randomly is shown below.
